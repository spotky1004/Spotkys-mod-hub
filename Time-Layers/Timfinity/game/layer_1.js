...
const SPACETIME_UPGRADE_COST = [[2,1.25,1.5,10,15,45,240,600,NaN,NaN,NaN,NaN],[2e6,20000,NaN,11,14,16,"e1050","e10500",NaN,NaN,NaN,NaN],[200,500,5000,10,20,NaN,NaN,NaN,NaN,1e15,1e37,'1e1304']]

function stinc(x,thing=false) {
  if (game.spaceless&&(!thing)&&x!==1&&x<=12) return false;
...
function getTempCompCost() {
  ...
  // tempTempCompCost.. xD
  var tempTempCompCost = EN(20).times(EN(1.5).pow(game.tempComp.pow(1.5))).div(1+2*stinc(13)).div(stinc(13)?getSpaceEnergyRow1Mult():1).div(stinc(14)?2.25*(game.SEU.includes(9)?10:1):1).times(inGalChal(4)?10:1);
  if (game.tempComp.gte(1e3)) {
    tempTempCompCost = tempTempCompCost.pow(game.tempComp.sub(1e3).div(100).add(1));
  }
  return tempTempCompCost;
}
...
function getTempCompEffect() {
  return getTempCompBase().pow(caplog10(game.tempComp).min(15).add(caplog10(getSpacetimeCompEffect())).add(inAnyGalChal()?0:game.superComp.temp)).times(game.tempComp.add(1))
}

function getTempCompBase() {
  return EN(1.1).add(
    game.tempComp.div(10)
    .times(
      (
        !stinc(310)?1:
        game.tempComp.div(10).add(1)
        .pow(
          (
            !stinc(311)?1:
            game.tempComp.div(15).add(1)
            .times(
              (
                !stinc(312)?1:
                game.tempComp.div(100).add(1)
              )
            )
          )
        )
  ...
  ).add(stinc(12)?(stinc(15)?caplog10square(game.spaceComp):game.spaceComp.min(12)).times(0.015):0).add(game.achievement.includes(28)?0.19:0)
}
