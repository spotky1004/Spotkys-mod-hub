...
function buyableDisplay(buyable, description) {
	const [LayerId, Id] = [buyable.layer, buyable.id];

	const Have = player[LayerId].buyables[Id];
	const Cache = temp[LayerId].buyables[Id];

	let output = "";
	output += description + "<br><br>"; // Description
    output += "effect: ".bold() + formatWhole(buyable.effect(Have)) + "x<br>"; // Effect
...
function complexCostDisplay(cost) {
	let output = "";
	output += "Cost: ".bold() + Object.entries(cost).map(e => `${formatWhole(e[1])} ${e[0]}`).join("<br>");
	return output;
}
...
function toNumber(x) {
	if (x.mag !== undefined) return x.floor().toNumber();
