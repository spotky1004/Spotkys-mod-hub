...
addLayer("ne", {
		...
		update(diff) {
			if (player.ne.unlocked && (player.ne.activeChallenge==11||hasAchievement("a", 151))) {
				player.ne.signals = player.ne.signals.plus(tmp.ne.challenges[11].amt.times(diff.mul(tmp.resMult))).min((hasMilestone("ne", 4)||hasMilestone("id", 0))?(1/0):tmp.ne.signalLim);
