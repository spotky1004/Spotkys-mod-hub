addLayer("p", {
		...
		passiveGeneration() { return (hasMilestone("g", 1)&&player.ma.current!="p")?1:0 + player.p.unlocked ? tmp.resPassive : 0 },
...
addLayer("e", {
		...
		passiveGeneration() { return (hasMilestone("q", 1)&&player.ma.current!="e")?1:0 + tmp.resPassive },
		update(diff) {
...
addLayer("h", {
		...
		passiveGeneration() { return (hasMilestone("m", 2)&&player.ma.current!="h")?1:0 + tmp.resPassive},
		milestones: {
			0: {
...
addLayer("q", {
		...
		passiveGeneration() { return (hasMilestone("ba", 0)&&player.ma.current!="q")?1:0 + tmp.resPassive},
		tabFormat: {
			"Main Tab": {
				content: [
					"main-display",
...
addLayer("o", {
	name: "solarity", // This is optional, only used in a few places, If absent it just uses the layer id.
		...
		passiveGeneration() { return player.ma.current=="o"?0:(hasMilestone("m", 0)?1:(hasMilestone("o", 0)?0.05:0)) + tmp.resPassive},
		solPow() {
			let pow = new Decimal(1);
...
addLayer("m", {
		...
		passiveGeneration() { return (hasMilestone("hn", 1)&&player.ma.current!="m")?1:0 + tmp.resPassive},
...
addLayer("ba", {
		...
		passiveGeneration() { return (hasMilestone("hn", 1)&&player.ma.current!="ba")?1:0 + tmp.resPassive },
...
addLayer("hn", {
		...
		passiveGeneration() { return (hasMilestone("ma", 1)&&player.ma.current!="hn")?1:0 + tmp.resPassive},
		canReset() {
...
addLayer("n", {
		...
		passiveGeneration() { return (hasMilestone("ma", 3)&&player.ma.current!="n")?1:0 + tmp.resPassive},
        gainExp() { // Calculate the exponent on main currency from bonuses
            return new Decimal(1)
        },
        row: 5, // Row the layer is in on the tree (0 is the first row)
...
addLayer("hs", {
		...
		passiveGeneration() { return (hasMilestone("ma", 2)&&player.ma.current!="hs")?1:0 + tmp.resPassive},
        doReset(resettingLayer){ 
			let keep = [];
...
addLayer("ge", {
		...
		passiveGeneration() { return hasMilestone("ge", 2)?0.01:0 + tmp.resPassive},
        doReset(resettingLayer){ 
			let keep = [];
			if (layers[resettingLayer].row > this.row) layerDataReset(this.layer, keep)
			if (layers[resettingLayer].row >= this.row) {
...
addLayer("mc", {
		...
		passiveGeneration() { return hasMilestone("mc", 0)?0.01:0 + tmp.resPassive},
        doReset(resettingLayer){ 
			let keep = [];
			if (layers[resettingLayer].row > this.row) layerDataReset(this.layer, keep)
        },
...
addLayer("en", {
		...
		canReset() {
			return  player.o.points.gte(tmp.en.req) && tmp.en.getResetGain.gt(0) && (hasMilestone("en", 0)||tmp.resMult.gt(1)?player.en.points.lt(tmp.en.getResetGain):player.en.points.eq(0))
		},
		dispGainFormula() {
...
addLayer("ne", {
		...
		update(diff) {
			if (player.ne.unlocked && (player.ne.activeChallenge==11||hasAchievement("a", 151))) {
				player.ne.signals = player.ne.signals.plus(tmp.ne.challenges[11].amt.times(diff.times(tmp.resMults))).min((hasMilestone("ne", 4)||hasMilestone("id", 0))?(1/0):tmp.ne.signalLim);
...
addLayer("ai", {
        ...
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1);
			mult = mult.mul(tmp.resMult.add(1).log(10));
		...
		passiveGeneration() { return tmp.resPassive },
        gainExp() { // Calculate the exponent on main currency from bonuses
            return new Decimal(1)
        },
        row: 6, // Row the layer is in on the tree (0 is the first row)
...
addLayer("ng", {
	...
	symbol: "NG",
	nodeStyle() { return {
		"animation": this.layerShown() ? "hueRotate 10s linear infinite" : "none"
	}},
	startData() { return {
		unlocked: false,
		points: new Decimal(0),
	...
	row: 7,
	position: 0,
	requires: new Decimal("e3.140e16"),
	layerShown() {return player.points.gt("e3.140e16") || player.ng.unlocked },
	color: "#d18484",
	...
	gainMult() {
		let mult = new Decimal(1);
		mult = mult.mul(player.ng.upgrades.includes(32) ? tmp.ng.upgrades[32].effect : 1 );
		return mult;
	},
	canReset() {
	...
	prestigeButtonText() {
		return `Reset for <b>${!this.canReset() ? "0" : "1"} NG</b> <br><br>Next at ${!this.canReset() ? format(new Decimal("e3.140e16").pow(player.ng.upgrades.includes(33) ? tmp.ng.upgrades[33].effect : 1)) : "Infinity"} point`
	},
	branches: ["ai", "ge", "ma", "mc", "c"],
	doReset(layer) {
		player.a.achievements = [];
		player.ma.mastered = [];
		player.ng.resetTime = player.timePlayed;
		for (const l of TREE_LAYERS.slice(0, 7).flat()) {
			player[l].milestones = [];
			player[l].unlocked = false;
		}
		for (let p = 0; p < Math.floor(player.ng.upgrades.includes(31) ? tmp.ng.upgrades[31].effect : 0); p++) {
			const i = (Math.floor(p/4)+1) + "" + (p%4+1);
			player.a.achievements.push(i);
		}
	},
	upgrades: {
		rows: 3,
		...
		11: {
			title: "Passive",
			description: "Gain <b>1%</b> of resources gain every second, Gain <b>10x</b> resources",
			cost: new Decimal(1)
		},
		12: {
			title: "Normal",
			description: "Gain <b>x100</b> resources",
			cost: new Decimal(1)
		},
		13: {
			title: "Active",
			description: "TBA..., Gain <b>10x</b> resources",
			cost: new Decimal(1)
		},
		21: {
			title: "Costless",
			description: "Upgrades won't consume resources, always max buy",
			cost: new Decimal(2)
		},
		22: {
			title: "Collector",
			description: "Totel NG multiplies resources",
			effect() {
		...
		23: {
			title: "Crescent",
			description: "Multiply resources gain based on unlocked layers",
			effect() {
		...
		31: {
			title: "Start Skip",
			description: "Retain achievements based on total NG",
			effect() {
				return Math.min(20, player.ng.total.add(1).log(1.5).toNumber());
			},
			effectDisplay() {
				const num = tmp.ng.upgrades[31].effect;
		...
		32: {
			title: "NGNG",
			description: "Multiply NG gain based on time spent on this NG",
			effect() {
				return new Decimal(10000).pow(Math.max(0, 2**((5000-(player.timePlayed-player.ng.resetTime))/500))/1024);
			},
			effectDisplay() {
				return `<b>${format(tmp.ng.upgrades[32].effect)}</b>x`
			},
			cost: new Decimal(7)
		},
		33: {
